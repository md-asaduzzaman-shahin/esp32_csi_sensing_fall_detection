...
typedef struct {
    ...
    float *data[CSI_CORR_NUM];                  /**< Array of training sample data pointers */
    ...
    float none_corr_sum;                         /**< Sum of correlation values for non-static environment */
    float none_corr_count;                       /**< Count of correlation values for non-static environment */
    float none_corr;                             /**< Current correlation value for non-static environment */
    float static_corr;                           /**< Static environment correlation value */
...
static esp_err_t csi_window_update(csi_data_buff_index_t *buff_index)
{
    ...
    if (time_tamp < 0 || time_tamp > (int32_t)(s_ctx.dec_config.csi_handle_time / 2)) {
        ESP_LOGW(TAG, "time_tamp: %d, spent_time: %d,csi_handle_time: %d, end: %d, last: %d, window: %d,handle_window: %d",
                 time_tamp, spent_time, s_ctx.dec_config.csi_handle_time, s_ctx.csi_data_buff->timestamp[buff_index->end], s_ctx.window_ctx.last_timestamp,
...
{
    uint8_t move_buffer_size = s_ctx.dec_config.pca_window_size;
    ...
    for (int i = 0; i < move_buffer_size - 1; ++i) {
        float corr_val = fabs(corr(pca_subcarrier_buff_current, pca_subcarrier_buff[(s_pca_subcarrier_buff_num - 2 - i) % move_buffer_size], cols));

        if (radar_info->waveform_jitter < corr_val) {
            radar_info->waveform_jitter = corr_val;
        }
    }
...
esp_err_t esp_radar_start()
{
    ...
    if (s_ctx.dec_config.csi_handle_time < s_ctx.csi_config.csi_recv_interval * (s_ctx.dec_config.pca_window_size)) {
        ESP_LOGE(TAG, "csi_handle_time is too short, will set to %d", s_ctx.csi_config.csi_recv_interval * (s_ctx.dec_config.pca_window_size));
        s_ctx.dec_config.csi_handle_time = s_ctx.csi_config.csi_recv_interval * (s_ctx.dec_config.pca_window_size);
    }
...
esp_err_t esp_radar_dec_init(esp_radar_dec_config_t *config)
{
    ...
    if (s_ctx.dec_config.pca_window_size < RADAR_PCA_WINDOW_DEFAULT) {
        ESP_LOGW(TAG, "pca_window_size < 2, fallback to default: %d", RADAR_PCA_WINDOW_DEFAULT);
        s_ctx.dec_config.pca_window_size = RADAR_PCA_WINDOW_DEFAULT;
    }
