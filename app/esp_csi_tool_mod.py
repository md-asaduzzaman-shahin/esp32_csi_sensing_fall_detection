...
def radar_data_handle(self, data):
    ...
    if g_display_eigenvalues_table:
        try:
            ...
            g_radar_data_human_pd.iloc[1:] = g_radar_data_human_pd.iloc[:-1].values # Fix for shifting rows
...
def serial_handle(queue_read, queue_write, port):
    try:
        set = serial.Serial(port=port, baudrate=115200,
    ...
    # Use a standard list of dictionaries instead of a DataFrame to ensure file handles are stored correctly
    data_valid_list = [
        ...
        for data_valid in data_valid_list:
            ...
            if index >= 0:
                ...
                if len(data) == len(data_valid['columns_names']):
                    ...
                    data = data[:len(data_valid['columns_names'])]
                    ...
                    if data_series['type'] == 'CSI_DATA':
                        ...
                        # FIX: Force the series to object type so it can accept the list
